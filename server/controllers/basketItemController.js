...
class BasketItemController {
    async add(req, res) {
        try {
            ...
            return res.json("Санаторий добавлен в избранное");
        } catch (e) {
            console.log(e);
        }
    }
    ...
    async delete(req, res) {
        try {
            ...
            await Basket.findOne({where: {userId: user.id}}).then(async userBasket => {
                ...
                return res.json(`Вы не можете удалить санаторий, потому что он не находится у вас в избранном`);
            });
            return res.json("Санаторий удален из избранного");
        } catch (e) {
            console.log(e);
        }
    }
